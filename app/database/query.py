...
def main():
    tests = """\
        ...
        select a, b from test_table where 1=1
        select z.a, b from test_table where 1=1
        select z.a, b from test_table where 1=1 order by b,c desc,d
    ...
    for test in tests.split('\n'):
        print(test)
        success, parsed = select_stmt.runTests(test)
        if not success:
            print("Error")
            print(parsed)
        print("----------------------------")
    ...
    # success, _ = select_stmt.runTests(tests)
    # print(select_stmt.parseString(" SELECT * FROM abcd WHERE 1!=2 and 2=3 or (foo='bar'"))
